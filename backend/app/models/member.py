...
class Member(Base):
    ...
    medical_conditions = Column(Text, nullable=True)  # Any medical conditions
    ...
    outstanding_dues = Column(
        Numeric(10, 2), default=0.00
    )  # Any amount that is due but not paid
    ...
    fees = relationship(
        "MemberFee", back_populates="member", cascade="all, delete-orphan"
    )

    __table_args__ = (
        Index(
            "unique_member_per_tenant",
            "tenant_id",
            "phone_number",
            unique=True,
