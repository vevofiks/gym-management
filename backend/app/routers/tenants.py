...
def update_my_tenant(
    ...
    try:
        ...
        logger.info(
            f"User {current_user.username} updated their tenant {current_user.tenant_id}"
        )
...
def get_my_subscription_status(
    ...
    """
    Get current tenant's subscription status, limits, and usage.
    """
    if not current_user.tenant_id:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    from app.services.subscription_service import get_subscription_status_detail

    return get_subscription_status_detail(db, current_user.tenant_id)
