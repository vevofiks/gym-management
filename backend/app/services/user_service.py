...
def create_user(
    ...
    final_tenant_id = (
        tenant_id if tenant_id is not None else getattr(user_create, "tenant_id", None)
    )
    ...
    db_user = User(
        ...
        role=(
            ...
            if hasattr(user_create.role, "value")
            else user_create.role
        ),
    )
...
def update_user_role(
    db: Session, user_id: int, new_role: UserRole, tenant_id: Optional[int] = None
) -> Optional[User]:
    """
    Update user role.
